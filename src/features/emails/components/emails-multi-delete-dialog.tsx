...
type EmailMultiDeleteDialogProps<TData> = {
  open: boolean
...
export function EmailsMultiDeleteDialog<TData>({
  open,
  onOpenChange,
  table,
}: EmailMultiDeleteDialogProps<TData>) {
    ...
    try {
      ...
      toast.promise(
        deleteEmails(idList),
        {
          ...
          success: () => `Deleted ${idList.length} ${idList.length > 1 ? 'emails' : 'email'}`,
          error: (err) => {
            // 盡可能把後端錯誤訊息顯示出來
    ...
    <ConfirmDialog
      ...
      title={
        <span className='text-destructive'>
          ...
          {selectedRows.length > 1 ? 'emails' : 'email'}
        </span>
      }
      desc={
        <div className='space-y-4'>
          <p className='mb-2'>
            Are you sure you want to delete the selected emails? <br />
